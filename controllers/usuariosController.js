...
exports.autenticarUsuario = async (req, res, next) => {
  ...
  if (!usuario || !usuario.activo) {
    ...
    try {
      await registrarAccion(usuario._id, "Inició sesión");
      res.json({ token });
    } catch (error) {
      console.error("Error al registrar la acción:", error);
      res
        .status(500)
        .json({ mensaje: "Hubo un error al registrar la acción", error });
    }
  }
};
//cerrar sesión
exports.logoutUsuario = async (req, res) => {
  try {
    ...
    await registrarAccion(req.usuario._id, "Cerró sesión");
    res.json({ mensaje: "Logout registrado correctamente" });
  } catch (error) {
    console.error("Error al registrar el logout:", error);
    res
      .status(500)
      .json({ mensaje: "Hubo un error al registrar el logout", error });
  }
};
